...
if(isset($_GET['email']))
{
    ...
    try {
		...
		$mail->isHTML(TRUE);
        $mail->Body = "<html><h1 style='background-color:gold;color:gray;text-align:center'>COMPTE ACTIVE AVEC SUCCES</h1><p style='text-align:center;color:white;background-color:slategray;'>Votre compte est desormais accesible grace a vos informations de connexion ... La Caverne a gouter vous remerci pour votre confiance. Faites des achats et debloquez votre compte de fidelite pour beneficier des reductions et d'autres avantages liés a votre compte. </p></html>";
        $mail->isSMTP();
        $mail->Port = 587;
        $mail->SMTPAuth = TRUE;
        $mail->SMTPSecure = 'tls';
